...
@section('ActivityUser', 'active-sub')
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <div class="panel">
                <div class="panel-body">
                    ...
                    <h4 class="text-main text-bold mar-no">Activity User</h4>
                    <p>&nbsp;</p>
                    ...
                    <div class="pad-btm form-inline">
                        <div class="row">
                            <div class="col-sm-6 table-toolbar-left">
                                <form method="GET" action="{{ route('activity-user.index') }}" class="form-inline">
                    ...
                    <div class="table-responsive">
                        <table id="table-depo" class="table table-striped table-bordered" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                @forelse ($activity as $items)
                                    <tr>
                                        <td>{{ ($activity->currentPage() - 1) * $activity->perPage() + $loop->iteration }}
                                        <td>{{ $items->created_date }}</td>
                                        <td>{{ $items->user->user_email }}</td>
                                        <td>{{ $items->ip_address }}</td>
                                        <td>{{ $items->ip_address_utama }}</td>
                                        <td>{{ $items->perangkat }}</td>
                                        <td>{{ $items->browser }}</td>
                                        <td>{{ $items->action }}</td>
                        ...
                        @if (!$activity->isEmpty())
                            <p class="d-inline-block">Showing {{ $activity->firstItem() }} to
                                {{ $activity->lastItem() }} of
                                {{ $activity->total() }} entries</p>
                        @endif
                        <hr class="new-section-xs">
                        <div class="pull-right">
                            <nav aria-label="Page navigation">

                                {{ $activity->appends(['perPage' => request()->get('perPage'), 'search' => request()->get('search')])->links('pagination::bootstrap-4') }}
                            </nav>
                        </div>
                    </div>
                </div>
